...
def export_to_pdf(modeladmin, request, queryset):
    ...
    for _prestations in dd:
        elements.extend(_build_dd(_prestations, queryset[0].invoice_number, queryset[0].invoice_date))
        elements.append(PageBreak())
...
#     for presta in queryset[0].prestations.all():
#         i+=1
...
#     elements.append(Paragraph("<b>Memoire d'Honoraires Num. %s en date du : %s</b>" %( invoiceNumber, invoiceDate), styles['Center']))
#     elements.append(Spacer(1, 18))
# 
...
def _build_dd(prestations, invoice_number, invoice_date):
    # Draw things on the PDF. Here's where the PDF generation happens.
    # See the ReportLab documentation for the full list of functionality.
    #import pydevd; pydevd.settrace()
    elements = []
